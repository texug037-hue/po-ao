...
app.v4.js              <- toda a lógica do app — este é o arquivo que muda a cada atualização
...
1. Vai vir um arquivo novo, tipo `app.v4.js` (nunca reaproveita o nome antigo).
2. Substitui o `app.v4.js` antigo pelo novo `app.v4.js` no repositório.
...
4. No `sw.js`, troca o número da versão do cache:
   ```js
   const CACHE_NAME = 'ponto-cache-v4';
   ```
   por:
   ```js
   const CACHE_NAME = 'ponto-cache-v4';
