...
**Volume 25** — três mudanças importantes:

1. **Corrigido bug sério de sincronização**: restaurar um backup (ou qualquer ação seguida de recarregar a página) podia perder os dados porque a página recarregava antes da nuvem confirmar o salvamento — a nuvem "vencia" com os dados antigos. Agora o app espera a confirmação da nuvem antes de recarregar.
2. **Toque no "PDV"** (cabeçalho) agora liga/desliga a visualização de preço de custo em todos os produtos da aba Marcas de uma vez — fica vermelho quando ativo.
3. **Categorias expandidas**: cada marca agora tem 7 categorias (Orais, Injetáveis, Blends Injetáveis, GHS, Peptídeos, Peptídeos Orais, Peptídeos Blends), cada uma com ícone e contador, expansível — em vez de só Oral/Injetável. Produtos já cadastrados nas categorias antigas continuam certos.

**Volume 24** — este zip já vem com as credenciais reais do Firebase preenchidas no firebase-config.js (projeto ponto-vendas-abbdf). ⚠️ Da próxima vez que subir uma atualização, não sobrescreva esse arquivo com um novo vazio — ele já está configurado.

**Volume 23** — adicionados avisos visíveis (toast) em toda a sincronização com o Firebase: agora aparece na tela "☁️ Dados restaurados da nuvem" quando funciona, ou uma mensagem de erro específica quando falha (autenticação, busca, ou salvamento) — antes esses erros só apareciam escondidos no console do navegador, invisíveis pra quem não é técnico. Isso ajuda a diagnosticar por que os dados não voltaram depois de limpar o aparelho.

**Volume 22** — toda marca agora tem duas subpastas fixas: 💊 Orais e 💉 Injetáveis, cada uma com seu próprio botão de adicionar item. Ao editar um item dá pra trocar a categoria se precisar. Itens antigos foram migrados automaticamente pelo nome (quem começa com "Oral" virou categoria oral, resto virou injetável) — confere se ficou certo, especialmente os que têm nome fora do padrão.

**Volume 21** — item com estoque zerado agora fica riscado (nome e preço) com uma etiqueta vermelha "Indisponível" no lugar do botão de adicionar, em vez de deixar tentar adicionar e dar erro. Continua dando pra segurar o dedo nele e editar pra repor o estoque — assim que a quantidade volta a ser maior que zero, ele volta ao normal sozinho.

**Volume 20** — adicionada a terceira opção no menu do ícone 🌐: "DG Importados" (planilha Google Sheets), junto com Respect Pharma e Medical Brasil.

**Volume 19** — quando toca no total pra ver o custo, os itens da lista expandida agora mudam junto (antes só o total trocava, os itens continuavam mostrando o valor de venda). Corrigido também: o Frete lançado no pedido tinha custo travado em zero — agora o custo dele bate com o valor (é repasse, sem markup), igual já valia no total geral da aba Itens.

**Volume 18** — tirado o texto "toque p/ ver custo" que aparecia embaixo do total dos pedidos. O toque continua funcionando normal (alterna venda/custo), só não escreve mais nada.

**Volume 17** — o total de cada pedido (nas abas Vendas e Histórico) agora é clicável, igual já funcionava em Itens: toque nele pra alternar entre o valor de venda e o custo total daquele pedido especificamente. Mostra "toque p/ ver custo" embaixo do valor pra ficar claro que dá pra alternar.

**Volume 16** — Frete agora entra no custo total também (era repasse, contava só no valor final). Lista de Itens agora agrupa por marca (ordem alfabética), e dentro de cada marca os itens também em ordem alfabética — antes vinha por valor, meio aleatório. Ícone 🌐 agora abre um menu pra escolher entre dois sites (Respect Pharma e Medical Brasil) em vez de ir direto pra um só.

**Volume 15** — corrigido de verdade: as marcas nunca tinham sido ordenadas alfabeticamente (só os itens dentro delas), agora sim. Tirado o indicador de bolinhas (o "botão amarelo") embaixo do total em Itens — o arrastar pro lado continua funcionando igual, só não mostra mais aquele indicador. O Frete agora aparece como uma linha de verdade na lista de Itens ("🚚 Frete R$ X"), sempre por último, quando ativado — não só como aviso embaixo do total.

**Volume 14** — clientes agora aparecem em ordem alfabética nas abas Clientes e Histórico (antes era por atividade mais recente). Marcas e itens dentro de cada marca já eram alfabéticos, continuam assim. Tudo que é listado por nome no app segue a mesma ordem A-Z agora.

**Volume 13** — corrigido: a bolinha de frete no cabeçalho (aba Itens) estava chamando a mesma função do frete por cliente e pedia pra selecionar um cliente antes. Agora ela é independente — só liga/desliga e soma o valor do frete direto no total mostrado ali em "Itens vendidos e pagos", sem depender de nenhum cliente ou pedido. O frete por cliente continua funcionando só pelo card em Marcas, separado.

**Volume 12** — Frete implementado de verdade: card fixo (sempre primeiro) no topo da aba Marcas, valor editável (toque no lápis), toque no card adiciona/remove do pedido ativo. Sempre aparece por último na lista do pedido, não importa quando foi adicionado. Bolinha vermelha/verde no cabeçalho, do lado do 💾, só aparece na aba Itens — mostra e controla se o pedido ativo tem frete. Ícone 🌐 do site, só aparece na aba Marcas, abre o link em nova aba. Corrigido também um bug que travava a finalização de pedidos com frete (achava que frete era produto sem estoque).

**Volume 11** — tirei a opção de "limpar tudo" (você não queria). Agora cada compra dentro do histórico do cliente tem um botão "🗑️ Excluir esta compra" visível, direto no card — apaga só aquela venda específica, sem mexer nas outras nem no cadastro do cliente.

**Volume 10** — achado o motivo real do cliente sumir: segurar o dedo no NOME do cliente e apertar "Excluir" apaga o cliente por completo (isso é intencional). Agora tem uma opção nova nesse mesmo menu — "🧹 Limpar histórico (mantém o cliente)" — que apaga só as compras antigas e mantém o cadastro do cliente intacto.

**Volume 9** — corrigido bug sério: restaurar um backup antigo (ou sincronizar com o Firebase pela primeira vez) podia apagar a lista de clientes sem querer, porque os dados vinham sem o cadastro novo de clientes e nada reconstruía isso. Agora todo lugar que troca os dados por inteiro (backup, nuvem) reconstrói o cadastro de clientes automaticamente, igual já acontecia ao abrir o app normal.
