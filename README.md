...
O app já vem preparado — enquanto você não mexer em nada, ele continua funcionando só com o armazenamento local (do jeito que já estava). Pra ligar a nuvem:
...
4. Aba **Regras** do Firestore, troca por (depois dá pra apertar a segurança):
   ```
   rules_version = '2';
   ...
   ```
5. Ícone de engrenagem → **Configurações do projeto** → desce até "Seus apps" → clica no ícone **`</>`** (Web) → registra um app (nome qualquer) → não precisa do Firebase Hosting.
6. Ele mostra um bloco `firebaseConfig = { apiKey: "...", ... }` — copia os valores de dentro pro `firebase-config.js`.
