...
function showItemActions(type, id){
  ...
  const obj = type==='brand' ? db.brands.find(x=>x.id===id) : db.products.find(x=>x.id===id);
  if(!obj) return;
  document.getElementById('itemActionsTitle').textContent = obj.name;
...
function actionEdit(){
  ...
  else openEditProductModal(contextTarget.id);
}
...
function actionDelete(){
  ...
  else deleteProduct(contextTarget.id);
}
