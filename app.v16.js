    ...
    <div class="order-card ${o.status==='pago'?'pago':''}"
      ...
      <div class="foot">
        <div onclick="event.stopPropagation(); toggleOrderCostView('${o.id}')" style="cursor:pointer">
          ...
          <div style="font-size:9.5px; color:var(--muted); font-family:var(--mono); text-transform:uppercase; letter-spacing:0.5px; margin-top:1px">${showCost ? 'custo · toque p/ voltar' : 'toque p/ ver custo'}</div>
        </div>
        ${actions}
        ${compactDeleteBtn}
      </div>
