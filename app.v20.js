...
function ensureDbShape(data){
  ...
  data.products.forEach(p=>{
    if(p.category !== 'oral' && p.category !== 'injetavel'){
      // migração de produtos antigos sem categoria — detecta pelo nome, "Oral" no começo vira oral
...
function renderBrands(){
  ...
  list.innerHTML = brandsSorted.map(b=>{
    ...
    const orais = allProds.filter(p=>p.category==='oral')
      .sort((a,b)=>a.name.localeCompare(b.name, 'pt-BR', {sensitivity:'base'}));
    const injetaveis = allProds.filter(p=>p.category==='injetavel')
      .sort((a,b)=>a.name.localeCompare(b.name, 'pt-BR', {sensitivity:'base'}));
    ...
    return `
      ...
      <div class="brand-body ${isOpen?'open':''}" id="brand-body-${b.id}">
        <div class="subfolder-title">💊 Orais</div>
        <div class="card">
          ${orais.length? orais.map(p=>productRow(p)).join('') : '<div class="empty" style="padding:14px 0">Nenhum item oral nesta marca.</div>'}
          <button class="btn-ghost btn-full" style="margin-top:8px" onclick="event.stopPropagation(); openProductModal('${b.id}','oral')">+ Adicionar oral</button>
        </div>
        <div class="subfolder-title">💉 Injetáveis</div>
        <div class="card">
          ${injetaveis.length? injetaveis.map(p=>productRow(p)).join('') : '<div class="empty" style="padding:14px 0">Nenhum item injetável nesta marca.</div>'}
          <button class="btn-ghost btn-full" style="margin-top:8px" onclick="event.stopPropagation(); openProductModal('${b.id}','injetavel')">+ Adicionar injetável</button>
        </div>
      </div>
    `;
  }).join('');
...
function selectProductCategory(cat){
  ...
  document.getElementById('catOralBtn').classList.toggle('active', cat==='oral');
  document.getElementById('catInjetavelBtn').classList.toggle('active', cat==='injetavel');
}
