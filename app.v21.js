...
const PRODUCT_CATEGORIES = [
  { key:'oral', label:'Orais', icon:'💊' },
  { key:'injetavel', label:'Injetáveis', icon:'💉' },
  { key:'blend_injetavel', label:"Blends (Mix's) Injetáveis", icon:'🧪' },
  { key:'ghs', label:'GHS', icon:'📈' },
  { key:'peptideo', label:'Peptídeos', icon:'🧬' },
  { key:'peptideo_oral', label:'Peptídeos Orais', icon:'🧬💊' },
  { key:'peptideo_blend', label:"Peptídeos Blends (Mix's)", icon:'🔗' },
];
const VALID_CATEGORY_KEYS = PRODUCT_CATEGORIES.map(c=>c.key);
let db = loadDb();
let activeBrandId = null;   // marca aberta no modal de "adicionar item"
let openBrandRow = null;    // marca expandida na aba Marcas
let marcasShowCost = false; // true = mostrando preço de custo em todos os produtos de Marcas

function toggleMarcasCostView(){
  marcasShowCost = !marcasShowCost;
  document.getElementById('pdvBadge').classList.toggle('cost-active', marcasShowCost);
  renderBrands();
}
...
function ensureDbShape(data){
  ...
  if(typeof data.itensFreteOn !== 'boolean') data.itensFreteOn = false;

  data.products.forEach(p=>{
    if(!VALID_CATEGORY_KEYS.includes(p.category)){
      // migração de produtos antigos sem categoria (ou de categoria removida) — detecta pelo nome, "Oral" no começo vira oral
      p.category = /^oral\b/i.test(p.name.trim()) ? 'oral' : 'injetavel';
    }
  });
  return data;
}
...
function updateHeader(){
  ...
  const hasFrete = db.itensFreteOn;
...
function renderClientList(){
  ...
  keys.sort((a,b)=> byCustomer[a].name.localeCompare(byCustomer[b].name, 'pt-BR', {sensitivity:'base'}));
...
let costViewOrders = new Set();

function toggleOrderCostView(id){
  if(costViewOrders.has(id)) costViewOrders.delete(id); else costViewOrders.add(id);
  refreshActiveView();
}
...
function orderCard(o, withActions, compact){
  ...
  const showCost = costViewOrders.has(o.id);
  ...
  const itemsDetail = isOpen && displayItems.length ? `
    <div class="items-detail">
      ${displayItems.map(i=>{
        const unitPrice = showCost ? i.cost : i.final;
        return `
        <div class="item-detail-row ${i.isFrete?'frete-row':''}">
          <span>${i.isFrete?'🚚 ':''}${i.qty}× ${escapeHtml(i.name)} <span class="u">(${brl(unitPrice)} un)</span></span>
          <span>${brl(unitPrice*i.qty)}</span>
        </div>
      `;}).join('')}
    </div>
  ` : '';
  ...
  return `
    <div class="order-card ${o.status==='pago'?'pago':''}"
      ...
      ${pixLine}
      <div class="foot">
        <div onclick="event.stopPropagation(); toggleOrderCostView('${o.id}')" style="cursor:pointer">
          <div class="tot" style="${showCost?'color:var(--text)':''}">${brl(showCost ? orderCost(o) : total)}</div>
        </div>
        ${actions}
        ${compactDeleteBtn}
      </div>
...
function toggleFrete(){
  ...
  if(has){
    ...
    o.items.push({ productId:'FRETE', name:'Frete', qty:1, cost: db.freteValue, final: db.freteValue, isFrete:true });
  }
  saveDb();
  renderFreteCard();
  updateHeader();
...
// bolinha do cabeçalho (só na aba Itens) — independente de cliente/pedido,
// só soma o valor do frete no total mostrado ali mesmo
function toggleItensFrete(){
  db.itensFreteOn = !db.itensFreteOn;
  saveDb();
  renderItens();
  updateHeader();
  toast(db.itensFreteOn ? 'Frete somado ao total' : 'Frete removido do total');
}
...
let openCategoryKey = null; // categoria expandida dentro da marca aberta no momento

function toggleCategory(catKey){
  openCategoryKey = (openCategoryKey===catKey) ? null : catKey;
  renderBrands();
}
...
function renderBrands(){
  ...
  const brandsSorted = [...db.brands].sort((a,b)=>a.name.localeCompare(b.name, 'pt-BR', {sensitivity:'base'}));
  list.innerHTML = brandsSorted.map(b=>{
    const allProds = db.products.filter(p=>p.brandId===b.id);
    const isOpen = openBrandRow===b.id;

    const categoriesHtml = PRODUCT_CATEGORIES.map(cat=>{
      const prods = allProds.filter(p=>p.category===cat.key)
        .sort((a,b)=>a.name.localeCompare(b.name, 'pt-BR', {sensitivity:'base'}));
      const catOpen = isOpen && openCategoryKey===cat.key;
      return `
        <div class="cat-row ${catOpen?'open':''}" onclick="event.stopPropagation(); toggleCategory('${cat.key}')">
          <span class="cat-icon">${cat.icon}</span>
          <span class="cat-label">${cat.label.toUpperCase()}</span>
          <span class="cat-count">${prods.length} produto${prods.length!==1?'s':''}</span>
          <span class="chev">›</span>
        </div>
        <div class="cat-body ${catOpen?'open':''}">
          <div class="card">
            ${prods.length? prods.map(p=>productRow(p)).join('') : `<div class="empty" style="padding:14px 0">Nenhum item em ${cat.label} nesta marca.</div>`}
            <button class="btn-ghost btn-full" style="margin-top:8px" onclick="event.stopPropagation(); openProductModal('${b.id}','${cat.key}')">+ Adicionar item</button>
          </div>
        </div>
      `;
    }).join('');
    ...
    return `
      <div class="brand-row ${isOpen?'open':''}" onclick="toggleBrand('${b.id}')"
        ...
        <div>
          ...
          <div class="count">${allProds.length} item${allProds.length!==1?'s':''}</div>
        </div>
        <div class="chev">›</div>
      </div>
...
function productRow(p){
  ...
  let control;
  if(outOfStock){
    control = `<span class="badge indisponivel">Indisponível</span>`;
  } else if(!canSell){
  ...
  return `
    <div class="product-item ${outOfStock?'out-of-stock':''}"
      ...
      <div class="right">
        <div class="pprice" style="${marcasShowCost?'color:#e88a8a':''}">${brl(marcasShowCost ? p.cost : p.final)}</div>
        ${control}
      </div>
    </div>
  `;
...
let activeProductCategory = 'injetavel';
...
function openProductModal(brandId, category){
  ...
  selectProductCategory(category || 'injetavel');
  updateProdPreview();
...
function openEditProductModal(productId){
  ...
  selectProductCategory(p.category || 'injetavel');
  updateProdPreview();
  openModal('modalProduct');
}
...
function submitProduct(ev){
  ...
  if(editingProductId){
    ...
    if(p){
      p.name = name; p.cost = cost; p.final = final; p.stock = stock; p.category = activeProductCategory;
    }
    toast('Item atualizado');
  } else {
    db.products.push({ id: uid(), brandId: activeBrandId, name, cost, final, stock, category: activeProductCategory });
...
function renderItens(){
  ...
  itensCache = Object.values(totals).sort((a,b)=>{
    const byBrand = a.brand.localeCompare(b.brand, 'pt-BR', {sensitivity:'base'});
  ...
  if(db.itensFreteOn){ totalFinal += db.freteValue; totalCost += db.freteValue; }
  ...
  document.getElementById('itensFreteNote').style.display = db.itensFreteOn ? 'block' : 'none';
...
function renderItensList(){
  ...
  if(itensCache.length===0 && !db.itensFreteOn){
  ...
  if(db.itensFreteOn){
    // frete sempre por último na lista — é repasse, não tem markup: mesmo valor no custo e no total
    const freteAmount = db.freteValue;
    html += `
      <div class="product-item" style="border-bottom:1px solid var(--line); padding:11px 4px">
        <div class="left">
          <div class="pname">🚚 Frete</div>
        </div>
        <div class="right">
          <div class="pprice" style="color:${priceColor}">${brl(freteAmount)}</div>
        </div>
      </div>
    `;
  }
...
function restoreBackupFile(ev){
  ...
  reader.onload = function(e){
    try{
      ...
      saveDb();
      toast('Backup restaurado — sincronizando com a nuvem...');
      const done = () => location.reload();
      if(typeof waitForCloudSync === 'function'){
        waitForCloudSync().then(done).catch(done);
        setTimeout(done, 4000); // segurança: recarrega de qualquer jeito se a nuvem demorar demais
      } else {
        done();
      }
    }catch(err){
