...
function orderCard(o, withActions, compact){
  ...
  const pixLine = (o.status==='pago' && o.pixAttachment)
    ? `<div class="pix-thumb" onpointerdown="event.stopPropagation()" onclick="viewPixAttachment('${o.id}')"><img src="${o.pixAttachment}" alt="comprovante"><span>ver comprovante</span></div>`
    : '';
...
function openEditProductModal(productId){
  ...
  document.getElementById('newProdMargin').value = p.margin;
...
function submitProduct(ev){
  ...
  if(editingProductId){
    ...
    if(p){
      p.name = name; p.cost = cost; p.margin = margin;
      p.final = calcFinal(cost, margin); p.stock = stock;
    }
...
function openEditOrderModal(orderId){
  ...
  document.getElementById('editOrderItems').innerHTML = o.items.map(it=>`
...
function deleteClient(clientName){
  ...
  const msg = `Excluir "${clientName}" e ${orders.length} pedido${orders.length>1?'s':''} dele(a)? Estoque já baixado volta automaticamente. Essa ação não pode ser desfeita.`;
  if(!confirm(msg)) return;
  orders.forEach(o=>{
    if(o.status!=='aberto') restoreStockForOrder(o);
