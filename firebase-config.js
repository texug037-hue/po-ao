...
function initFirebaseSync(){
  ...
  if(typeof firebase === 'undefined'){
    if(typeof toast === 'function') toast('⚠️ SDK do Firebase não carregou (sem internet?)');
    return;
  }
  try{
    ...
    firebase.auth().signInWithEmailAndPassword(FIREBASE_AUTH_EMAIL, FIREBASE_AUTH_PASSWORD)
      ...
      .catch(err => {
        if(typeof toast === 'function') toast('⚠️ Erro ao conectar na nuvem: ' + err.message);
    ...
    if(typeof toast === 'function') toast('⚠️ Firebase indisponível — usando só armazenamento local');
...
function startFirestoreSync(){
  ...
  fbDocRef.get().then(snap=>{
    if(snap.exists && snap.data().payload){
      ...
      db = ensureDbShape(JSON.parse(snap.data().payload));
      localStorage.setItem(STORAGE_KEY, JSON.stringify(db));
      if(typeof refreshActiveView === 'function') refreshActiveView();
      if(typeof updateHeader === 'function') updateHeader();
      fbApplyingRemote = false;
      if(typeof toast === 'function') toast('☁️ Dados restaurados da nuvem');
    } else {
      // nuvem ainda vazia — manda o que já tem localmente pra lá
      if(typeof toast === 'function') toast('☁️ Nuvem vazia — enviando dados locais');
      pushToCloud(true);
    }
  }).catch(err=>{
    if(typeof toast === 'function') toast('⚠️ Erro ao buscar dados da nuvem: ' + err.message);
  ...
  fbDocRef.onSnapshot(snap=>{
    ...
    fbApplyingRemote = true;
    db = ensureDbShape(JSON.parse(data.payload));
    localStorage.setItem(STORAGE_KEY, JSON.stringify(db));
    if(typeof refreshActiveView === 'function') refreshActiveView();
    if(typeof updateHeader === 'function') updateHeader();
    fbApplyingRemote = false;
  }, err=>{
    if(typeof toast === 'function') toast('⚠️ Conexão em tempo real caiu: ' + err.message);
    console.warn('onSnapshot erro:', err.message);
  });
}

function pushToCloud(immediate){
  if(!fbReady || fbApplyingRemote) return Promise.resolve();
  clearTimeout(fbSaveTimer);
  const send = () => fbDocRef.set({
    payload: JSON.stringify(db),
    updatedAt: firebase.firestore.FieldValue.serverTimestamp()
  }).catch(err=>{
    if(typeof toast === 'function') toast('⚠️ Não salvou na nuvem: ' + err.message);
    console.warn('Não consegui salvar na nuvem agora:', err.message);
  });
  if(immediate) return send();
  return new Promise(resolve=>{
    fbSaveTimer = setTimeout(()=>{ send().then(resolve); }, 700); // agrupa mudanças rápidas numa escrita só
  });
}

// espera a nuvem confirmar antes de fazer algo que dependa disso (ex: recarregar a página) —
// se o Firebase não estiver ativo, resolve na hora e segue o fluxo normal
